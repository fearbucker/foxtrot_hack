...
function addIncomeEntry() {
    ...
    const incomeHTML = `
        <div class="entry" id="income-${incomeEntryCount}">
            ...
            <button type="button" onclick="removeIncomeEntry(${incomeEntryCount})" class="remove-button">Remove</button>
        </div>`;
...
function addExpenseEntry() {
    ...
    const expenseHTML = `
        <div class="entry" id="expense-${expenseEntryCount}">
            ...
            <button type="button" onclick="removeExpenseEntry(${expenseEntryCount})" class="remove-button">Remove</button>
        </div>`;
...
function removeIncomeEntry(entryId) {
    const incomeEntry = document.getElementById(`income-${entryId}`);
    incomeEntry.remove();
}

function removeExpenseEntry(entryId) {
    const expenseEntry = document.getElementById(`expense-${entryId}`);
    expenseEntry.remove();
}
